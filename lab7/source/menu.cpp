...
template <typename T> void Menu::saveToText(std::vector<T> &list) {
    if (list.empty()) {
        std::cout << "List is empty. Nothing to save.\n";
        return;
    }
    ...
    std::string type = list.front().machine_type().c_str();
    std::string fname = std::string("files/") + type + std::string(".txt");
    try {
        File f(fname);
        f.open(std::ios::out | std::ios::trunc);
        f.file << list.front().text_header() << '\n';
        for (const T &item : list) {
            item.to_text_row(f.file);
            f.file << '\n';
        }
        f.close();
        std::cout << "Saved to " << fname << "\n";
...
template <typename T> void Menu::loadFromText(LinkedList<T> &list) {
    // allow loading into empty list by using default-constructed T to infer type
    T proto;
    std::string type = proto.machine_type().c_str();
    std::string fname = std::string("files/") + type + std::string(".txt");
    try {
        File f(fname);
        f.open(std::ios::in);
...
template <typename T> void Menu::loadFromText(std::vector<T> &list) {
    // allow loading into empty container by using default-constructed T to infer type
    T proto;
    std::string type = proto.machine_type().c_str();
    std::string fname = std::string("files/") + type + std::string(".txt");
    try {
...
template <typename T> void Menu::saveToBinary(std::vector<T> &list) {
    if (list.empty()) {
        std::cout << "List is empty. Nothing to save.\n";
        return;
    }
...
template <typename T> void Menu::loadFromBinary(std::vector<T> &list) {
    // allow loading into empty container by using default-constructed T to infer type
    T proto;
    std::string type = proto.machine_type().c_str();
    std::string fname = std::string("files/") + type;
...
void Menu::run() {
    int choice = 0;
    // ask user which container to use
    std::cout << "Choose container type: 1) std::vector  2) custom LinkedList\n";
    std::cout << "Choice: ";
    int container_choice = input_int(std::cin, 1, 2);
    use_vector = (container_choice == 1);
    do {
        ...
        case 1:
            if (use_vector)
                familyMenu<SmartPhone>(smartphones_v, "SmartPhone");
            else
                familyMenu<SmartPhone>(smartphones, "SmartPhone");
            break;
        case 2:
            if (use_vector)
                familyMenu<Laptop>(laptops_v, "Laptop");
            else
                familyMenu<Laptop>(laptops, "Laptop");
            break;
        case 3:
            if (use_vector)
                familyMenu<Monoblock>(monoblocks_v, "Monoblock");
            else
                familyMenu<Monoblock>(monoblocks, "Monoblock");
            break;
        case 4:
            if (use_vector)
                familyMenu<PortableMachine>(portables_v, "PortableMachine");
            else
                familyMenu<PortableMachine>(portables, "PortableMachine");
            break;
        case 5:
            if (use_vector)
                familyMenu<PersonalComputer>(pcs_v, "PersonalComputer");
            else
...
template <typename T> void Menu::familyMenu(std::vector<T> &list, const std::string &name) {
    int choice = 0;
    do {
        std::cout << "\n=== " << name << " menu ===\n";
        std::cout << "1) Add device\n";
...
template <typename T> void Menu::addDevice(std::vector<T> &list) {
    std::cout << "Enter new device fields via input operator (>>):\n";
    T obj;
    clearStdin();
    if (!(std::cin >> obj)) {
...
template <typename T> void Menu::printDevices(std::vector<T> &list) {
    if (list.empty()) {
        std::cout << "List is empty.\n";
        return;
    }
    list.front().print_header();
    std::cout << std::endl;
    for (const T &item : list) {
...
template <typename T> void Menu::editDevice(std::vector<T> &list) {
    if (list.empty()) {
        std::cout << "List is empty.\n";
        return;
    }
...
template <typename T> void Menu::deleteDevice(std::vector<T> &list) {
    if (list.empty()) {
        std::cout << "List is empty.\n";
        return;
    }
...
template <typename T> void Menu::sortDevices(std::vector<T> &list) {
    if (list.size() < 2) {
        std::cout << "Not enough elements to sort.\n";
        return;
    }
    ...
    switch (choice) {
    case 1:
        std::sort(list.begin(), list.end(), [](const T &a, const T &b) { return a.get_cpu() < b.get_cpu(); });
        break;
    case 2:
        std::sort(list.begin(), list.end(), [](const T &a, const T &b) { return a.get_operating_system() < b.get_operating_system(); });
        break;
    default:
        std::cout << "Unknown sort option.\n";
        return;
...
template <typename T> void Menu::searchDevices(std::vector<T> &list) {
    if (list.empty()) {
        std::cout << "List is empty.\n";
        return;
    }
    ...
    case 1: {
        String q(query.c_str());
        auto it = std::find_if(list.begin(), list.end(), [&q](const T &val) { return val.get_cpu() == q; });
        if (it != list.end())
    ...
    case 2: {
        String q(query.c_str());
        auto it = std::find_if(list.begin(), list.end(), [&q](const T &val) { return val.get_operating_system() == q; });
        if (it != list.end())
            std::cout << "Found: " << *it << std::endl;
        else
            std::cout << "Not found.\n";
